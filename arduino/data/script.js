...
// API Configuration
const LAMPY_API = {
    base: "/api",  // Use relative path so it works with any IP address
    endpoints: {
        status: "/status",
        update: "/update",
        discover: "/discover",
        wifi: "/wifi",
        bluetooth: "/bluetooth",
        mode: "/mode",  // Add mode endpoint for backward compatibility
    },
};
...
createApp({
    data() {
        return {
            ...
            // Cached LED positions to avoid recalculating every frame
            cachedLEDPositions: [],
            ...
            switch (this.connectionStatus) {
                case "connected":
                    return this.selectedDevice
                        ? `Connected to ${this.selectedDevice.name}`
                        : "Connected";
                case "connecting":
                    return "Connecting...";
                case "disconnected":
    ...
    mounted() {
        ...
        this.patterns = Object.entries(PATTERN_DEFINITIONS).map(
            ([id, pattern]) => ({
                id: parseInt(id),
                ...pattern,
            })
    ...
    methods: {
        ...
        async initializeLampy() {
            try {
                const response = await fetch(
                    `${LAMPY_API.base}${LAMPY_API.endpoints.status}`
                );
                if (response.ok) {
        ...
        async scanForDevices() {
            ...
            try {
                const response = await fetch(
                    `${LAMPY_API.base}${LAMPY_API.endpoints.discover}`,
                    {
                        method: "POST",
                    }
                );
        ...
        resetColors() {
            if (this.currentPattern && this.currentPattern.originalColors) {
                this.currentPattern.colors = [
                    ...this.currentPattern.originalColors,
                ];
                this.applySettingsLive();
            }
        },

        // Precalculate LED positions to avoid heavy calculations every frame
        precalculateLEDPositions() {
            const totalLEDs = 72;
            const tubeHeight = 500;
            const tubeRadius = 60;
            const tubeHeightMobile = 340;
            const tubeRadiusMobile = 40;
        ...
        // LED Positioning - Using cached positions
        ...
        getLEDColor(ledIndex) {
            ...
            switch (pattern.id) {
                case 0: // Fire
                    const fireBase =
                        Math.sin(time * speed * 3 + ledIndex * 0.2) *
                        Math.cos(time * speed * 2 + ledIndex * 0.15);
                    const fireFlicker = Math.random() * 0.3;
                    const fireIntensity = Math.max(
                        0,
                        (fireBase + fireFlicker) * 0.5 + 0.4
                    );
                    const fireColorIndex = Math.min(
                        Math.floor(fireIntensity * colors.length),
                        colors.length - 1
                    );
                ...
                case 2: // Rainbow
                    ...
                    const rainbowColorIndex = Math.floor(
                        rainbowPos * colors.length
                    );
                    color = colors[rainbowColorIndex] || "#ff0000";
                    intensity =
                        brightness *
                        (0.8 +
                            Math.sin(time * speed * 2 + ledIndex * 0.1) * 0.2);
                    break;
                ...
                case 3: // Fireflies
                    ...
                    const pulseBase =
                        Math.sin(time * speed + ledIndex * 0.3) * 0.5 + 0.5;
                    const sparkleMultiplier = isNewSpark ? 2 : 1;
                    intensity =
                        brightness * pulseBase * sparkleMultiplier * 0.6;
                    color =
                ...
                default: // Waves (Asters, Ocean, Kelp, Mandarin)
                    ...
                    const wave2 =
                        Math.sin(wavePhase * 0.7 + Math.PI / 3) * 0.3 + 0.5;
                    const combinedWave = (wave1 + wave2) / 2;
                    const waveColorIndex = Math.floor(
                        combinedWave * colors.length
                    );
                    color =
                        colors[Math.min(waveColorIndex, colors.length - 1)] ||
                        "#00ff00";
        ...
        async sendToLampy() {
            ...
            if (this.connectionStatus === "connected") {
                try {
                    const response = await fetch(
                        `${LAMPY_API.base}${LAMPY_API.endpoints.update}`,
                        {
                            method: "POST",
        ...
        // Animation Loop with frame rate limiting and cleanup
        startAnimationLoop() {
            let lastFrameTime = 0;
